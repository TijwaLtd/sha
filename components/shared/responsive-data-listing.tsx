...
export interface GroupConfig<T> {
  key: (item: T) => string;
  header?: (group: ListingGroup<T>) => React.ReactNode;
}
...
export function ResponsiveDataListing<T>({
  ...
  const buildGroups = (): ListingGroup<T>[] => {
    if (!groupBy) return [{ key: '', items }];
    const groups: ListingGroup<T>[] = [];
      ...
      {loading ? (
        ...
        <>
                    ...
                    <React.Fragment key={group.key || '__default__'}>
                      {groupBy?.header && (
                        <tr className="border-b border-border/80 bg-muted/40">
                          <td colSpan={columns.length} className="px-4 py-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
          ...
          <div className="space-y-4 md:hidden">
            {groups.map((group) => (
              <div key={group.key || '__default__'} className="space-y-3">
                {groupBy?.header && (
                  <div className="px-1">{groupBy.header(group)}</div>
                )}
                {group.items.map((item) => {
                  const k = rowKey(item);
                  return (
